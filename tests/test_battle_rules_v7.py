"""Battle v11：达妮娅、阿萨姆、熠～噜猪及当前对战版本的聚焦规则验收。"""
...
def test_v8_catalog_has_exact_new_daniya_asamu_and_yilu_definitions():
    ...
    assert DANIYA_COMMON_MOVES[2].resolved_opponent_reduction_tenths == 521
...
def test_daniya_domain_weight_modifiers_and_timed_collapse_are_exact():
    ...
    finish(left)
    ready(right)
    record(right, DANIYA_COMMON_MOVES[-1], 1)
    finish(right)
...
def test_asamu_new_growth_chain_dynamic_weights_and_two_domain_copies():
    ...
    assert move_weight_units(player, ASAMU_MOVES[1]) == 1500
    apply_move(player, ASAMU_MOVES[1])
    assert move_weight_units(player, ASAMU_MOVES[1]) == 1000
    assert move_weight_units(player, ASAMU_MOVES[3]) == 300
    ...
    assert charge["gain"] == 5 and move_weight_units(player, ASAMU_MOVES[3]) == 1300
    prime = apply_move(player, ASAMU_MOVES[3])
    assert prime["gain"] == 35 and move_weight_units(player, ASAMU_MOVES[3]) == 300

    player["injury_state"] = "none"
    assert move_weight_units(player, ASAMU_MOVES[5]) == 500
    player["injury_state"] = "light"
    assert move_weight_units(player, ASAMU_MOVES[5]) == 1000
    player["injury_state"] = "heavy"
    assert move_weight_units(player, ASAMU_MOVES[5]) == 2000

    domain = {"hit_side": 0, "domain_ids": [["asamu-domain"], []]}
...
def test_yilu_markers_babel_redeploy_operator_cap_and_true_damage():
    ...
    caster["yilu_markers"] = 6
    ready(caster)
    event = record(caster, YILU_MOVES[1], 0)
    finish(caster)
    finish(true_state["sides"][1])
    assert event["yilu_consumed_markers"] == 7 and event["yilu_true_damage_added"] == 1
...
def test_yilu_domain_hope_uses_real_clash_or_solo_hit():
    ...
    ready(clash["sides"][1])
    record(clash["sides"][1], DANIYA_COMMON_MOVES[-1], 1)
