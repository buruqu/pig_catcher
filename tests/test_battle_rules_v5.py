...
def _daniya_domain_state(*, opponent="sukuna") -> tuple[dict, tuple]:
    ...
    # 达妮娅普通领域=3，宿傩=4，领域战内部统一按10倍整数权重抽签。
    return current, (("side-0", 30), ("side-1", 40), ("tie", 30))
...
def test_daniya_solo_domain_hit_switches_form():
    ...
    assert domain["mode"] == "solo"
    assert domain["boost_side"] == 0 and domain["boost_reason"] == "领域命中"
    assert domain["bonus_gain"] == 30
    assert summary["interactions"]["daniya_transition"]["after"] == DANIYA_FORM_DISILLUSION
    assert summary["before"][0]["daniya_form"] == DANIYA_FORM_DISILLUSION
    assert summary["before"][0]["next_action_bonus"] == 1
...
def test_unified_invalidation_zeroes_daniya_own_gain_but_keeps_directed_effects():
    ...
    attacker["daniya_form"] = DANIYA_FORM_DISILLUSION
    ready(attacker)
    event = record(attacker, DANIYA_DISILLUSION_MOVES[1], 1)
    finish_turn(attacker)
...
def test_asamu_sleep_adds_five_to_later_moves_and_charge_only_buffs_prime_weight():
    player = state(left="asamu")["sides"][0]
    charge, hit, sleep = ASAMU_MOVES[4], ASAMU_MOVES[0], ASAMU_MOVES[2]
    ready(player, 4)
...
def test_asamu_domain_clash_win_copies_exactly_two_opponent_moves_deterministically():
    def source():
        current = state(left="asamu", right="sukuna")
        ready(current["sides"][0])
    ...
    wheel = (("side-0", 30), ("side-1", 40), ("tie", 30))
    ...
    assert len(copies) == 2
    assert [event["copy_slot"] for event in copies] == [1, 2]
...
def test_asamu_solo_domain_hit_doubles_domain_and_copies_two_opponent_moves():
    current = state(left="asamu", right="sukuna")
    ready(current["sides"][0])
    domain_event = record(current["sides"][0], ASAMU_MOVES[-1], 0)
    finish_turn(current["sides"][0])
    finish_turn(current["sides"][1])
    wheel = (("hit", 8), ("simple-domain", 2))
    seed = seed_for("1:domain:solo:0", wheel, "hit", prefix="asamu-solo-domain")
    summary = resolve_round(current, seed)
    domain = summary["interactions"]["domain"]
    copies = summary["interactions"]["asamu_domain_copies"]
    assert domain["boost_side"] == 0 and domain["boost_reason"] == "领域命中"
    assert domain["boosted_ordinals"] == [domain_event["ordinal"]]
    assert domain["bonus_gain"] == 22
    assert len(copies) == 2
    assert all(event["generated_by"] == "asamu-domain-copy" for event in copies)
