...
def _validate_session_ids(values: list[str]) -> list[str]:
    """Normalize MaiBot command-routing scopes or legacy session IDs."""
...
class AccessSection(PluginConfigBase):
    ...
    command_session_allowlist: list[str] = Field(
        default_factory=list,
        max_length=500,
        description="仅向指定 MaiBot 聊天会话注册本插件的命令组件",
        json_schema_extra=_ui(
            "命令作用域白名单",
            "用于双版本灰度；优先填写 platform:group_id，也兼容稳定 session_id。"
            "留空表示全局注册，再由群白名单判断。修改后需重启 MaiBot。",
        ),
    )
