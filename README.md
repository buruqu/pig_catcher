...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.5.0`；本轮在保留 BanG Dream! 联动猪猪、群专属六星素材和全部原命令的基础上，增加了可审计的运营控制面板。
...
- 交付阶段：`6`；当前组件为 31 个显式 `COMMAND` 和 1 个管理首页 `HOME_CARD`，不注册普通消息监听、Tool 或 LLM
...
- 169 项正式模板：124 只猪、45 道美食，其中 12 项动画、28 项群专属；每个已授权群作用域可见 117 只猪和 38 道美食
- GIF 与动画 WebP 逐帧卡片合成，保留帧数、时长和循环，不改写原始素材
- 32 只 BanG Dream 联动猪的角色、乐队、官方资料与固定 `X/5` 收集进度；Poppin'Party、Afterglow、Pastel＊Palettes、MyGO!!!!!、Morfonica 与 Hello, Happy World! 均已完整收集 `5/5`；梦限大动画薇欧拉为单槽特别联动；米歇尔猪为乐队吉祥物熊
- 155 份唯一正式媒体、169 项模板及备用图的路径、哈希、尺寸、格式和逐帧解码检查
- 图片生成或发送失败后的纯文字降级服务
