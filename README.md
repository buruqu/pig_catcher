...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.2.2`，基础六轮开发、经济成长扩展、首轮概率调平与新素材扩充已经完成。
...
- 数据协议：Schema `6`、Asset Manifest `3`、Ruleset `5`
...
- 群与用户黑白名单，黑名单优先
- 插件自有 SQLite Schema 6、逐级迁移、外键、WAL、显式事务和在线备份
...
- 112 项正式素材：88 只猪、24 道美食，其中 9 项动画、6 项群专属
- GIF 与动画 WebP 逐帧卡片合成，保留帧数、时长和循环，不改写原始素材
- 9 只 BanG Dream 联动猪的角色、乐队、官方资料与固定 `X/5` 收集进度；Afterglow 已完整收集 `5/5`
- 112 项逐图检查、9 项逐帧动画检查及两个群的六星可见性实测
- 图片生成或发送失败后的纯文字降级服务
...
- 六档做菜矩阵、肥瘦食谱池、厨具加成，以及六星猪基础 `90% 五星 / 10% 六星` 规则；专用一次性美食效果可把下一次六星菜概率提升到清单声明值
- 美食详情、背包、图鉴和食用；低星菜提供主要经验收益，高星菜可排队一次性抓猪、做菜、品质、体型或额外次数效果
- 猪猪商城单页展示全部 8 种消耗品与 2 种永久升级；消耗品用 `/购买`，猪饲料与厨具用 `/升级`，均支持 `Lv.0-5`
...
- 每小时生产巡检 SQLite 完整性、全库账本、112 个启用素材文件和过期报价
- 抓取、档案、猪与美食详情、背包、图鉴、纪录、商城、账本和经济回执的白粉图片
- 单猪和单道美食 GIF/动画 WebP 保持动态；背包、图鉴和排行榜使用确定性的中间帧 PNG 预览，原始动画字节不变
