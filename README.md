...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.4.2`；新增精确到群成员 OpenID 的永久赠送/收赠与交易黑名单，以及可自动到期的每时段抓猪额度处罚。
...
- 数据协议：Schema `10`、Asset Manifest `4`、Ruleset `12`
...
- 群与用户黑白名单，黑名单优先
- 插件自有 SQLite Schema 10、逐级迁移、外键、WAL、显式事务和在线备份
...
- 159 项正式模板：116 只猪、43 道美食，其中 12 项动画、24 项群专属；每个已授权群作用域可见 110 只猪和 37 道美食
- GIF 与动画 WebP 逐帧卡片合成，保留帧数、时长和循环，不改写原始素材
- 26 只 BanG Dream 联动猪的角色、乐队、官方资料与固定 `X/5` 收集进度；Poppin'Party、Afterglow、Pastel＊Palettes、MyGO!!!!! 与 Morfonica 均已完整收集 `5/5`；梦限大动画薇欧拉为单槽特别联动
- 146 份唯一正式媒体检查、10 项逐帧动画检查及四个群作用域的六星可见性实测
- 图片生成或发送失败后的纯文字降级服务
...
- 每小时生产巡检 SQLite 完整性、全库账本、159 个启用模板媒体和过期报价
