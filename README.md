...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.10.0`；本轮新增按群隔离、
带不可变账本与审计回执的插件管理员命令。
...
- 数据协议：Schema `15`、Asset Manifest `4`、Ruleset `14`
...
- 群与用户黑白名单，黑名单优先
- 插件自有 SQLite Schema 15、逐级迁移、外键、WAL、显式事务和在线备份
...
- 管理员发放资产沿用当前群已授权模板并生成全库唯一短编号，不伪造抓猪/做菜统计或奖励；管理员删除采用 `admin-removed` 终态，保留历史实例与图鉴
