...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.7.0`；本轮完成全量审计，
修复批量保留语义与联动猪保护，并把四个 QQ 群作用域纳入强制同步回归。
...
- 数据协议：Schema `14`、Asset Manifest `4`、Ruleset `12`
...
- 10 种道具定义；抓猪与做菜道具均可装备、取消，并只在兼容动作成功提交后原子消耗
- 六档做菜矩阵、肥瘦食谱池、厨具加成，以及六星猪基础 `90% 五星 / 10% 六星` 规则；若产出六星，成品只能是该猪模板绑定的同群定制六星菜
- 美食详情、背包、图鉴和食用；低星菜提供主要经验收益，高星菜可排队一次性抓猪、做菜、品质、体型或额外次数效果
- 猪猪商城单页展示全部 10 种消耗品与 2 种永久升级；新增超级幸运猪哨（5/6 星实际概率各精确 `×5`，其余档等比例压缩）与超级主厨香料（六星菜概率额外 `+5` 个百分点），同动作只能装备一种道具
- 六星菜基础价值为 `25,000`，做菜奖励为 `1,500` 猪币和 `800` 经验，吃菜经验为 `1,200`；定制效果覆盖下一次六星猪 `50%`、下一次六星菜最高 `60%`、本周每时段 `+5` 和永久每时段 `+1`（上限 `+5`）
- 周额度与永久额度在基础分时额度上生效；普通“额外次数”仍只消费当日一次性次数，三者独立显示和结算
...
北京时间次日零点失效。
不同菜品的效果互不重复；同一互斥组（抓猪概率组、做菜概率组）内最多生效最早的一个，
其余在结果中注明“互斥未叠加”。菜品效果可与道具和等级加成共用；抓猪/做菜结果图与
文字摘要均展示最终概率及来源（等级、饲料/厨具、道具、美食加成）。
