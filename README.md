...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.6.8`；新增“批量保留”开关：批量售卖与批量做菜默认保留一只最高价值联动猪，开启后额外保留一只最高价值普通猪和一只最高价值美食；批量做菜默认只处理一至三星低星猪。
...
- 数据协议：Schema `12`、Asset Manifest `4`、Ruleset `12`
- 交付阶段：`6`；当前组件为 33 个显式 `COMMAND` 和 1 个管理首页 `HOME_CARD`，不注册普通消息监听、Tool 或 LLM
- 当前群聊命令：`/抓猪帮助`、`/抓猪`（别名 `/抓群友`）、`/抓猪档案`、
  ...
  `/切换 猪保千`（在保千猪立绘与猪保千表情包之间切换）；
  `/批量售卖 猪猪 三星`、`/批量做菜 三星` 等可按品质过滤批量处理（一星至五星）；
  `/开启批量保留`、`/关闭批量保留`（开启后批量操作额外保留一只价值最高的猪猪与美食；
  联动猪始终默认保留一只价值最高者）
...
- 群与用户黑白名单，黑名单优先
- 插件自有 SQLite Schema 12、逐级迁移、外键、WAL、显式事务和在线备份
...
- 每小时生产巡检 SQLite 完整性、全库账本、169 个启用模板媒体和过期报价
