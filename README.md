...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.6.6`；新增“群级窗口提额”运营能力：管理员可将指定群当前时段额度提升（如 15 次，含违规者），窗口切换后自动恢复每时段 5 次。
...
- 182 项正式模板：135 只猪、47 道美食，其中 12 项动画、36 项群专属（NapCat 双群 + QQ 官方双群）；每个 NapCat 授权群可见 123 只猪和 39 道美食，QQ 官方群可见 123 只猪和 39 道美食
...
- 162 份唯一正式媒体、182 项模板及备用图的路径、哈希、尺寸、格式和逐帧解码检查
- 图片生成或发送失败后的纯文字降级服务
