...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.3.1`，基础六轮开发、经济成长扩展、分时额度、群素材强隔离与六星定制菜一对一结算已经完成。
...
- 数据协议：Schema `7`、Asset Manifest `4`、Ruleset `9`
- 交付阶段：`6`；当前组件为 29 个显式 `COMMAND` 和 1 个管理首页 `HOME_CARD`，不注册普通消息监听、Tool 或 LLM
...
- 猪猪商城单页展示全部 8 种消耗品与 2 种永久升级；消耗品用 `/购买`，猪饲料与厨具用 `/升级`，并同时列出幸运猪哨、主厨香料和两类永久升级的实际概率
- 六星菜基础价值提升到 `25,000`，做菜奖励提升到 `1,500` 猪币和 `800` 经验，吃菜经验提升到 `1,200`；三种定制效果分别强化到迷你偏移 `-0.50`、下一次六星菜 `60%`、下一次六星猪相对权重 `×12`
...
只在下一次兼容动作成功提交时消耗。额外抓猪次数只在超过当前时段基础额度后扣除，并在
北京时间次日零点失效。

需要人工重置时，可在 MaiBot 首页点击“重置抓猪次数”，进入插件配置的“额度重置”
分区，填写精确群号、打开一次性开关并保存；也可直接运行
