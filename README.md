...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.9.1`；本轮修复等级、猪饲料
与概率道具组合时四星权重挤压五星/六星的问题，并同步强化厨具与等级的普通做菜成长。
...
- 数据协议：Schema `14`、Asset Manifest `4`、Ruleset `14`
- 交付阶段：`6`；当前组件为 35 个显式 `COMMAND` 和 1 个管理首页 `HOME_CARD`，不注册普通消息监听、Tool 或 LLM
...
- 群与用户黑白名单，黑名单优先
- 插件自有 SQLite Schema 14、逐级迁移、外键、WAL、显式事务和在线备份
