...
这是“抓猪插件”的独立 MaiBot 插件仓库。当前正式版本为 `1.8.0`；本轮完成 4/5 星菜、
全部商城道具、额度菜与六星独占规则的统一数值平衡，并让每次抓猪/做菜结果展示最终概率。
...
- 210 项正式模板：147 只猪、63 道美食，其中 12 项动画、64 项群专属（NapCat 双群 + QQ 官方双群）；每个授权群可见 123 只猪和 39 道美食
- GIF 与动画 WebP 逐帧卡片合成，保留帧数、时长和循环，不改写原始素材
- 38 只 BanG Dream 联动猪的角色、乐队、官方资料与固定 `X/N` 收集进度；Poppin'Party、Afterglow、Pastel＊Palettes、MyGO!!!!!、Morfonica 与 Roselia 完整收集 `5/5`，Hello, Happy World! 含米歇尔猪完整收集 `6/6`；梦限大动画薇欧拉为单槽特别联动
- 162 份唯一正式媒体、210 项模板及备用图的路径、哈希、尺寸、格式和逐帧解码检查
- 图片生成或发送失败后的纯文字降级服务
...
- 每小时生产巡检 SQLite 完整性、全库账本、210 个启用模板媒体和过期报价
