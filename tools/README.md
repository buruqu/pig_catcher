...
当前内部测试基线：`2.0.3 / Schema49 / Ruleset44`，85个显式 Command + 1个 HOME_CARD。
...
`accept_battle_views.py` 生成完整状态流图卡、DOM诊断与联系表；回合结算图含双方本回合全部招式，使用临时合成数据和已有公开素材；
...
报告记录实际组件数量，不再把历史命令数写死为加载门禁；当前85+1不代表85个无别名的文字入口。
