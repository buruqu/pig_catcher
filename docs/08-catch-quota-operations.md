# 抓猪额度运维手册

## 1. 当前规则

所有时间固定使用北京时间 `Asia/Shanghai`：

| 当前窗口 | 每群每位玩家基础额度 | 下一次刷新 |
| --- | ---: | --- |
| 00:00–09:00 | 5 次 | 09:00 |
| 09:00–12:00 | 5 次 | 12:00 |
| 12:00–19:00 | 5 次 | 19:00 |
| 19:00–次日 00:00 | 5 次 | 次日 00:00 |

同一窗口内，成功抓猪后冷却 `20` 秒。进入新窗口或完成群级手动重置后，上一窗口或
重置前的成功回执不再参与当前计数和冷却。历史回执、资产、图鉴、账本与终身抓取统计
始终保留。

## 2. 菜品临时提额层

Ruleset 13 提供两个语义不同且可同时生效的普通菜品额度层：

| 菜品 | 生效范围 | 到期时间 | 重复食用 |
| --- | --- | --- | --- |
| 一盒油炸猪（四星） | 仅当前额度窗口 `+2` | 当前窗口结束 | 当前窗口内拒绝并保留菜品 |
| 猪寿司拼盘（五星） | 今天当前及后续每个额度窗口各 `+2` | 北京时间次日 00:00 | 当天拒绝并保留菜品 |

两层可以相加；滚动 7 天每时段额度、永久每时段额度和历史兼容的全天共享次数池也分别独立
计算。额度处罚始终拥有最高优先级，不能用任何菜品或道具绕过。配置面板的群级临时提额
仍按当前窗口覆盖普通额度显示，窗口切换后自动失效。

六星菜“向你道早猪猪巧克力螺”的每时段 `+5` 从食用时刻起滚动 7 天，不再按周一刷新；
到达七天周年时刻后，该时刻所在的额度窗口仍完整生效，窗口结束时失效。小马猪蒙布朗、
雾蓝键盘大福和猪保千猪排轮盘使用专属抓猪次数，不扣这里统计的普通窗口额度；但处罚期
硬上限仍按全部成功抓猪计算。

这两种菜进入现有可到期效果队列，不新增 Schema。玩家在两个 NapCat 群和两个 QQ 官方
群中的效果、额度和资产仍分别持久化，不会因为实际群一一对应而跨作用域同步。

## 3. 收到“重置某群抓猪次数”时

只提取用户明确指定的群号，不猜测、不使用全局重置。仓库根目录执行：

```powershell
uv run python .\tools\reset_catch_quota.py --group-id <精确群号>
```

生产默认数据目录会自动解析为
`C:\Users\Administrator\MaiBot\data\plugins\local.pig-catcher`。非默认环境必须显式增加：

```powershell
--data-dir C:\path\to\local.pig-catcher
```

命令严格按以下顺序执行：

1. 验证数据库中存在 `qq:<群号>` 范围；不存在时拒绝操作。
2. 使用 SQLite 在线备份生成 `backups/pig_catcher-pre-quota-reset-*.sqlite3`。
3. 统计该群当前窗口内、上次有效重置后的成功抓取次数和玩家数。
4. 写入群级 `catch-quota-window-reset` 审计事件。
5. 运行 `PRAGMA quick_check` 并以 JSON 输出结果。

重置通过审计时间戳改变“有效窗口起点”，不删除或改写任何抓猪回执。

额度重置备份与自动备份、其他操作前备份共用同一保留池；默认只保留插件数据目录
`backups/` 中最新 7 份 `.sqlite3`。这里生成的是重置前的一致在线副本，不是永久发布保护点；
需要长期保留时，应在核验通过后复制到插件运行 `backups/` 目录之外。

## 4. 群内管理员命令

在需要重置的群内，由“访问控制 → 插件管理员”中已配置的成员发送：

```text
/重置
```

命令不接收群号，固定重置当前消息所在群。执行顺序与命令行一致：先验证管理员和群范围，
再在线备份、写入群级审计事件，并发送归零次数与涉及人数的文字摘要。同一消息重复投递时
不会再次备份、重置或公示。

玩家通过糖醋排骨获得的 `/重置额度` 使用相同备份、群范围、审计和幂等边界，并额外在
同一事务中为当前群已登记玩家各写入 1,007 猪币账本、创建 10 次/人的群级专属额度和
5/6 星 `×1.007`。效果到当前窗口起点加一天时失效；使用专属额度的抓猪回执额度成本为 0，
并显示糖醋排骨发动者的 QQ 昵称，不显示数字 QQ 或 OpenID。每次实际使用糖醋排骨专属额度时独立进行一次
`10%` 隐藏判定；命中只把本次 5/6 星倍率从 `×1.007` 替换为 `×10.04`，结果摘要显示
“隐藏效果爆发”，随机值与最终概率写入回执快照。额度检查失败、冷却未结束或事务回滚时
不消费专属次数；违规额度处罚仍是硬上限，不能被专属额度绕过。

食用糖醋排骨时先发布一次“取得发动资格”全群通告，并明确尚未执行重置。食用者真正发送
`/重置额度` 且事务成功后，再发布“全群额度重置完成”通告，展示昵称、归零次数、受影响人数、
全群猪币、专属次数、基础倍率和隐藏倍率。两次公示分别复用两条业务消息的幂等回执；同一消息
重投不会重复公示，图片故障会发送完整事件文字。管理员 `/重置` 不触发该六星盛宴通告。

NapCat 接入可在管理员列表填写数字 QQ 号；QQ 官方机器人不会提供数字 QQ 号，必须填写
该机器人的成员 OpenID，推荐使用 `qq-official:<成员OpenID>` 形式。

### 4.1 只重置一名玩家

配置管理员还可以在目标玩家所在群发送：

```text
/猪管重置玩家 <@玩家|用户ID>
```

这条命令只把该玩家当前时段的有效抓猪次数和冷却归零，不影响同群其他玩家，也不删除
抓取回执、资产或终身统计。实现方式是写入玩家级
`player-catch-quota-window-reset` 审计事件；同一平台消息 ID 重投不会重复执行。
目标必须已经在当前群留下插件身份，QQ 官方接入应使用结构化 `@` 或该群成员 OpenID，
不能填写数字 QQ 号或仅凭昵称猜测。该命令不生成全库备份；其业务变更、审计和命令回执
在同一事务提交。其他群内管理员能力见
[抓猪运营控制面板手册](09-admin-panel-operations.md#7-群内插件管理员命令)。

## 5. MaiBot 管理面板

MaiBot 首页的“抓猪运营管理”卡片包含“重置抓猪次数”入口。点击后：

1. 在“额度重置”分区填写一个精确群号。
2. 打开“重置当前时段”一次性开关。
3. 保存配置。

插件会完成与命令行相同的备份和审计式重置，并在成功后把开关自动恢复为关闭。群号
保留，方便下次核对；再次重置仍必须重新打开开关并保存。

同一卡片还提供赠送/交易黑名单和群公告入口，详见
[抓猪运营控制面板手册](09-admin-panel-operations.md)。

## 6. 核验

命令行返回应满足：

- `status` 为 `ok`；
- `scope_id` 等于期望的 `qq:<群号>`；
- `integrity_check` 为 `["ok"]`；
- `backup_path` 文件在本次操作完成后存在；后续维护可能按统一保留数轮转该文件；
- `cleared_catches` 与本次希望归零的有效次数一致。

数据库审计查询：

```sql
SELECT audit_event_id, scope_id, actor_user_id, action, detail_json, created_at
FROM audit_events
WHERE action = 'catch-quota-window-reset'
ORDER BY created_at DESC
LIMIT 20;
```

禁止直接删除 `command_receipts`、清空 `player_statistics` 或修改玩家资产来达到重置效果。

## 7. 双接入群作用域说明

同一个实际 QQ 群通过 NapCat 与 QQ 官方 Bot 接入时会形成不同 `scope_id`，额度与玩家数据
不会自动合并。当前对应关系为：

| 实际群 | NapCat 范围 | QQ 官方范围 |
| --- | --- | --- |
| 第一组 | `qq:1092931381` | `qq-official:5E5854406D0297D6FEAE696A13E3A339` |
| 第二组 | `qq:237716658` | `qq-official:9EA2810F378FBD7DC3219C56CEAB3520` |

重置额度时必须操作消息实际进入的范围，不能把“一一对应”理解为两个范围共用计数。
群专属猪猪/美食内容会同步四份，但额度、资产、账本和玩家偏好仍分别持久化。
